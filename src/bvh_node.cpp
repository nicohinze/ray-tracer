...
bool axis_compare(std::shared_ptr<Hittable>& obj1, std::shared_ptr<Hittable>& obj2, int axis) {
    auto bb1 = obj1->bounding_box(0, 0);
    auto bb2 = obj2->bounding_box(0, 0);
    return bb1->get_min()[axis] < bb2->get_min()[axis];
}

bool x_axis_compare(std::shared_ptr<Hittable>& obj1, std::shared_ptr<Hittable>& obj2) {
    return axis_compare(obj1, obj2, 0);
}

bool y_axis_compare(std::shared_ptr<Hittable>& obj1, std::shared_ptr<Hittable>& obj2) {
    return axis_compare(obj1, obj2, 1);
}

bool z_axis_compare(std::shared_ptr<Hittable>& obj1, std::shared_ptr<Hittable>& obj2) {
    return axis_compare(obj1, obj2, 2);
}

std::atomic<std::uint64_t> BVH_Node::intersection_tests = 0;

BVH_Node::BVH_Node(std::vector<std::shared_ptr<Hittable>>& objects, float t0, float t1)
    : BVH_Node(objects, 0, objects.size(), t0, t1) {
}

BVH_Node::BVH_Node(std::vector<std::shared_ptr<Hittable>>& objects, std::size_t start, std::size_t end, float t0, float t1) {
    auto copy = objects;
...
std::optional<Intersection> BVH_Node::intersect(const Ray& ray) const {
    ...
    if (const auto intersect = aabb.intersect(ray)) {
        ...
        } else if (left_intersect.has_value()) {
            return left_intersect;
        } else if (right_intersect.has_value()) {
