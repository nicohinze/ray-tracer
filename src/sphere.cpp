...
std::optional<Intersection> Sphere::intersect(const Ray& ray) const {
    ...
    auto u1 = glm::dot(ray.direction, v);
    ...
    if (radius * radius >= d_2) {
        ...
        auto t1 = u1 + u2;
        if (t0 >= 0) {
            return Intersection(t0, ray.direction * t0, glm::normalize(ray.direction * t0 - center), material);
        }
        if (t1 >= 0) {
            return Intersection(t1, ray.direction * t1, glm::normalize(ray.direction * t1 - center), material);
        }
    }
    return std::nullopt;
}
